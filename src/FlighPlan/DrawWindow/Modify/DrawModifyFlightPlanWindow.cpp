...
void DrawModifyFlightPlanWindow(HDC hdc,int ox, int oy,EuroScopePlugIn::CFlightPlan fp,EuroScopePlugIn::CRadarTargetPositionData rtPos,double scale,int  modifyView,RECT& outViewRect,RECT& outModifyRect) {
    COLORREF WINDOW_BG            = RGB(77,  77,  77);
    ...
    const COLORREF ABLE_FILL      = RGB(250, 250, 250);
    ...
    std::string aircraftTypeStr = GetAircraftType(fp);
    ...
    fieldable(162, 20, 25,  14, "A/C",                    aircraftTypeStr.c_str());
    field(193, 20, 10,  14, "W",                      Wake.c_str());
